...
export default {
  ...
  methods: {
    ...
    editorConfig(editor) {
      ...
      //配置语言为zh-cn/en
      if (this.config && this.config.useLang && E.langs[this.config.useLang]) {
        editor.config.lang = E.langs[this.config.useLang];
      }
      ...
      editor.config.uploadImgFns.onload = function(resultText, xhr, fileName) {
        ...
        if (fileUrl) {
          ...
          uploadError.call(
            this,
            fileName,
            this.config.lang.uploadErrorPlaceTxt
          );
        }
      };
...
.wangEditor-fullscreen .wangEditor-menu-container {
  position: fixed;
  top: 0;
  left: 0;
}
